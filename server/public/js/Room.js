...
let swalBackground = 'linear-gradient(to right, black, black, #2F4F29)'; //'rgba(0, 0, 0, 0.7)';
...
function getPeerInfo() {
    peer_info = {
        ...
        peer_id: socket.id,
        peer_name: peer_name,
        peer_email: peer_email,  // 🔄 ✅ NEW FIELD
 ...
 if (idFromQuery) {
    ...
    `,
        ...
        preConfirm: () => {
            ...
            setCookie(room_id + '_email', email, 30);
            peer_name = name;
            peer_email = email;
        }
    }).then(async () => {
...
function joinRoom(peer_name, room_id, peer_email) {
    if (rc && rc.isConnected()) {
        ...
        // ✅ Optional: Display email if you want (you can hide it if not needed in UI)
        if (typeof userEmail !== 'undefined') {
            userEmail.innerText = peer_email || '';
        }

        isUserPresenter.innerText = isPresenter;

        // ✅ Add peer_email when creating RoomClient
        rc = new RoomClient(
            localAudio,
            remoteAudios,
            videoMediaContainer,
...
function setCustomTheme() {
    ...
    swalBackground = `linear-gradient(to right, black, black, #2F4F29)`;
...
function setTheme() {
    ...
    switch (theme) {
        case 'default':
            swalBackground = 'linear-gradient(to right, black, black, #2F4F29)';
            document.documentElement.style.setProperty('--body-bg', 'linear-gradient(to right, black, black, #2F4F29)');
...
function showAbout() {
    ...
    Swal.fire({
        ...
        title: BRAND.about?.title && BRAND.about.title.trim() !== '' ? BRAND.about.title : 'WebRTC SFU v1.8.21',
        html: `
            <br />
            <div id="about">
