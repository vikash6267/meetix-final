...
function startServer() {
    ...
    */
    ...
    const openai = new OpenAI({ apiKey: config?.integrations?.chatGPT?.apiKey });
...
app.post("/save-silent-recording", upload.single("audio"), async (req, res) => {
    ...
    app.get('/session/:sessionID', async (req, res) => {
        const sessions = await Session.find({ sessionId: req.params.sessionID });
        res.json(sessions);
    });
    ...
    app.post('/login', async (req, res) => {
        ...
        try {
            ...
            const secret = jwtCfg.JWT_KEY || 'special_meet_secret';
    ...
    app.get('/get-recordings', async (req, res) => {
        try {
            ...
            // List all blobs in the container and fetch properties
            ...
            recordings.sort((a, b) => new Date(b.properties.createdOn) - new Date(a.properties.createdOn));

            // Send the sorted list of recordings as response
            res.status(200).json({
            ...
            res.status(500).send('An error occurred while fetching the recordings.');
        }
    });
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('join', async (dataObject, cb) => {
            ...
            const newAttendee = new Attendee({
                ...
                peerEmail: data.peer_info.peer_email,
                peerId: socket.id,
                joinTime: new Date(),
                isHost: isPeerHost,
            });
 ...
 if (room.getPeersCount() === 0) {
    ...
    delete presenters[socket.room_id];
        ...
        socket.on('message', async (dataObject) => {
            if (!roomExists(socket)) return;
            ...
            const chat = new ChatMessage({
                roomId: socket.room_id,
                senderName: data.peer_name,
                senderId: socket.id,
                message: data.peer_msg,
            });
